...
internal sealed partial class AccountManagerForm : Form
{
    ...
    private readonly Label _count = AccountUiTheme.Label(UiText.T("내 계정"), 14, true);
    private readonly Label _status = AccountUiTheme.Label("");
    private readonly Button _add = AccountUiTheme.Button("AddAccountButton", UiText.T("+ 계정 추가"));
    private readonly Button _refreshAll = AccountUiTheme.Button("RefreshAllUsageButton", UiText.T("전체 갱신"), true);
    private readonly Button _register = AccountUiTheme.Button("RegisterCurrentButton", UiText.T("현재 계정 등록"), true);
    private readonly Button _registerActive = AccountUiTheme.Button("RegisterActiveAccountButton", UiText.T("현재 계정 등록"));
    private readonly ToolStripMenuItem _rename = new(UiText.T("이름 변경")) { Name = "RenameAccountButton" };
    private readonly ToolStripMenuItem _readUsage = new(UiText.T("이 계정 사용량 조회")) { Name = "ReadAccountUsageButton" };
    private readonly ToolStripMenuItem _delete = new(UiText.T("저장된 로그인 삭제")) { Name = "DeleteAccountButton" };
    private readonly ToolStripMenuItem _refresh = new(UiText.T("저장된 계정 목록 다시 읽기")) { Name = "RefreshAccountsButton" };
    private readonly ToolStripMenuItem _info = new(UiText.T("상세 정보")) { Name = "AccountInfoMenuItem" };
    private readonly ContextMenuStrip _menu = new();
    private readonly ContextMenuStrip _pageMenu = new();
    private readonly Button _recover = AccountUiTheme.Button("RecoverAccountsButton", UiText.T("미완료 전환 복구"));
    private readonly Button _cancel = AccountUiTheme.Button("CancelLoginButton", UiText.T("조회 취소"));
    ...
    private readonly CombinedUsagePanel _overview = new() { Dock = DockStyle.Fill, Margin = new Padding(0) };
    ...
    internal AccountManagerForm CreateLocalizedCopy() => new(_store, _suspend, _resume, _accountsChanged,
        _queryUsage, refreshAllOnOpen: false, combined: _combined, usageChanged: _usageChanged);
        ...
        Func<SavedCodexAccount, CancellationToken, Task>? queryUsage = null, bool refreshAllOnOpen = true, CombinedUsageSnapshot? combined = null, Action? usageChanged = null)
    {
        ...
        AccountUiTheme.SetForm(this); Text = UiText.T("Codex 계정 관리");
        ClientSize = new Size(920, 740); MinimumSize = new Size(780, 550); StartPosition = FormStartPosition.CenterScreen; KeyPreview = true;
        _root.Padding = new Padding(30, 30, 30, 20);
        foreach (var h in new[] { 82, 154, 0, 56 }) _root.RowStyles.Add(new RowStyle(SizeType.Absolute, h));
        _root.RowStyles.Add(new RowStyle(SizeType.Percent, 100)); _root.RowStyles.Add(new RowStyle(SizeType.Absolute, 30));
        ...
        header.Controls.Add(AccountUiTheme.Label(UiText.T("계정 관리"), 26, true), 0, 0);
        _add.AutoSize = _refreshAll.AutoSize = false;
        _add.Size = _refreshAll.Size = _add.MinimumSize = _refreshAll.MinimumSize = new Size(110, 38);
        ...
        _count.Anchor = AnchorStyles.Left; listHeader.Controls.Add(_count, 0, 0);
        var order = AccountUiTheme.Label(UiText.T("초기화가 가까운 순"), 9, color: AccountUiTheme.Muted); order.Anchor = AnchorStyles.Right;
        ...
        var footer = AccountUiTheme.Label(UiText.T("ⓘ  창을 열 때 한 번 확인하며, 이후에는 직접 갱신합니다."), 9, color: AccountUiTheme.Muted);
        footer.Margin = new Padding(0, 10, 0, 0); _root.Controls.Add(footer, 0, 5); Controls.Add(_root); Controls.Add(_progress);
        foreach (var menu in new[] { _menu, _pageMenu }) { menu.BackColor = AccountUiTheme.Surface; menu.ForeColor = AccountUiTheme.Text; menu.Font = Font; menu.ShowImageMargin = false; menu.Renderer = new ToolStripProfessionalRenderer(new AccountMenuColors()); }
        ...
        _refresh.Click += (_, _) => { if (!_busy && Reload()) SetStatus(UiText.T("저장된 계정 목록을 다시 읽었습니다.")); };
        ...
        _cancel.Click += (_, _) => { _loginCancellation?.Cancel(); _cancel.Enabled = false; SetStatus(UiText.T("요청을 취소하고 로그인 정보를 안전하게 정리하고 있습니다…")); };
        ...
        // Load runs after DPI scaling but before the first visible paint. Shown was
        // too late: empty controls and each intermediate row size reached the screen.
        Load += async (_, _) =>
        {
            var initialRead = Task.CompletedTask;
            Present(() =>
            {
                ...
                if (_refreshAllOnOpen && _items.Count > 0 && !_store.HasPendingRecovery && _usageQueryStatus.State == UsageQueryState.None) initialRead = ReadAllUsageAsync();
            });
            await initialRead;
        };
        FormClosing += (_, e) =>
        {
            ...
            if (_batchQuerying) { _closeAfterBatch = true; _loginCancellation?.Cancel(); SetStatus(UiText.T("전체 조회를 취소하고 안전하게 정리한 뒤 닫습니다…")); }
            else SetStatus(UiText.T("진행 중인 작업이 있습니다. 취소 버튼으로 요청을 마친 뒤 닫아주세요."));
        };
        FormClosed += (_, _) => _refreshTimer.Stop(); ResumeLayout(true);
        DpiChanged += (_, _) => BeginInvoke((Action)(() => { if (!IsDisposed) UpdateActions(); }));
    }
    private Task RegisterCurrentAsync() => RunAsync(true, UiText.T("현재 계정을 등록하고 있습니다…"), async () =>
    {
        var account = _store.RegisterCurrent(NextName()); Reload(account.Id);
        SetStatus(UiText.F($"‘{account.Label}’ 등록 완료. 계정의 ··· 메뉴에서 이름을 바꿀 수 있습니다."), success: true);
    ...
    private void BuildEmpty()
    {
        ...
        layout.Controls.Add(AccountUiTheme.Label(UiText.T("첫 계정을 등록하세요"), 18, true), 0, 0);
        var description = AccountUiTheme.Label(UiText.T("지금 사용하는 Codex 계정을 저장하면 시작할 수 있습니다.\n이름은 나중에 바꿀 수 있고, 현재 로그인은 유지됩니다."), color: AccountUiTheme.Muted);
    ...
    private void ShowAccountInfo()
    {
        ...
        using var dialog = new Form { Name = "AccountInfoDialog", Text = UiText.T("계정 상세"), ClientSize = new Size(460, 280), StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false };
        ...
        layout.Controls.Add(AccountUiTheme.Label(account.Label, 16, true), 0, 0); layout.Controls.Add(AccountUiTheme.Label(UiText.Message(account.IdentityHint), color: AccountUiTheme.Muted), 0, 1);
        var shortWindow = account.Usage?.ShortWindow; var now = DateTimeOffset.Now;
        var remaining = shortWindow is null ? UiText.T("정보 없음") : shortWindow.ResetsAt <= now ? UiText.T("갱신 필요") : $"{100 - shortWindow.UsedPercent}%";
        layout.Controls.Add(AccountUiTheme.Label(UiText.F($"5시간 잔여  {remaining}\n초기화  {(shortWindow?.ResetsAt is { } reset ? reset.ToLocalTime().ToString("MM/dd HH:mm") : "—")}")), 0, 2);
        layout.Controls.Add(AccountUiTheme.Label(account.ObservedAt is { } observed ? UiText.F($"개별 조회의 마지막 확인  {observed.ToLocalTime():MM/dd HH:mm}\n전체 잔여량은 마지막 전체 확인 기준입니다.") : UiText.T("아직 사용량을 확인하지 않았습니다."), color: AccountUiTheme.Muted), 0, 3);
        var close = AccountUiTheme.Button("CloseAccountInfoButton", UiText.T("닫기")); close.DialogResult = DialogResult.OK; layout.Controls.Add(close, 0, 4); dialog.AcceptButton = dialog.CancelButton = close;
    ...
    private void UpdateActions() => Present(UpdateActionState);
    private void Present(Action update)
    {
        using var layout = AccountUiTheme.DeferLayout(_root);
        update();
    }
    private void UpdateActionState()
    {
        var pending = _store.HasPendingRecovery || _usageQueryStatus.State == UsageQueryState.RecoveryRequired;
        // Disable actions individually. Disabling the entire table recursively
        // forces every transparent label to repaint its ancestors immediately.
        _refresh.Enabled = !_busy; _readUsage.Enabled = !_busy && !pending && Selected is not null;
        ...
        _recover.Text = _store.HasPendingRecovery ? UiText.T("미완료 전환 복구") : _usageQueryStatus.State == UsageQueryState.CleanupPending ? UiText.T("임시 파일 정리") : UiText.T("중단된 조회 복구");
        _refreshAll.Enabled = !_busy && !pending && _items.Count > 0;
        _overview.Display(_combined, _busy || pending, _batchCompleted, _batchQuerying);
        _accounts.Display(_combined, _busy, !pending, _items.Any(a => a.IsActive), keepOrder: _batchQuerying); UpdateNotice(); _usageChanged?.Invoke();
    }
    private string NextName()
    {
        for (var n = 1; ; n++)
        {
            var candidate = UiText.F($"계정 {n}");
            if (!_items.Any(a => a.Label == candidate)) return candidate;
        }
    }
    ...
    private void RenameSelected()
    {
        ...
        if (dialog.ShowDialog(this) != DialogResult.OK) { SetStatus(UiText.T("이름 변경을 취소했습니다.")); return; }
        try { _store.Rename(account.Id, dialog.AccountName); Reload(account.Id); _accountsChanged?.Invoke(); SetStatus(UiText.T("계정 이름을 변경했습니다."), success: true); }
    ...
    private async Task AddAccountAsync()
    {
        ...
        if (dialog.ShowDialog(this) != DialogResult.OK) { SetStatus(UiText.T("계정 추가를 취소했습니다.")); return; }
        var name = string.IsNullOrWhiteSpace(dialog.AccountName) ? NextName() : dialog.AccountName;
        await RunAsync(true, UiText.T("브라우저에서 추가할 계정으로 로그인하세요. 현재 계정은 유지됩니다."), async () =>
        {
            using var cancellation = new CancellationTokenSource();
            _loginCancellation = cancellation; _cancel.Text = UiText.T("로그인 취소"); _cancel.Visible = true; _cancel.Enabled = true;
            try
            {
                ...
                SetStatus(UiText.F($"‘{account.Label}’ 추가 완료. 목록에서 선택해 전환할 수 있습니다."), success: true);
            }
    ...
    {
        if (_busy || Selected is not { } account) return;
        await RunAsync(false, UiText.F($"‘{account.Label}’ 사용량을 조회하고 있습니다…"), async () =>
        {
            using var cancellation = new CancellationTokenSource();
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(30));
            using var linked = CancellationTokenSource.CreateLinkedTokenSource(cancellation.Token, timeout.Token);
            _loginCancellation = cancellation; _cancel.Text = UiText.T("조회 취소"); _cancel.Visible = true; _cancel.Enabled = true;
            try
            {
                ...
                Reload(account.Id);
                SetStatus(UiText.F($"‘{account.Label}’ 사용량을 확인했습니다."), success: true);
            }
        ...
        }, acquireGate: account.IsActive, canceledMessage: UiText.T("사용량 조회를 취소했습니다. 마지막 확인값은 유지됩니다."));
    }

    private async Task SwitchSelectedAsync()
    {
        if (_busy || Selected is not { IsActive: false } target) return;
        await RunAsync(true, UiText.T("전환 조건을 확인하고 있습니다…"), async () =>
        {
            var source = _items.FirstOrDefault(a => a.IsActive)?.Label ?? UiText.T("현재 로그인");
            using var dialog = new AccountSwitchDialog(source, target.Label);
            if (dialog.ShowDialog(this) != DialogResult.OK) { SetStatus(UiText.T("전환을 취소했습니다. 현재 계정은 유지됩니다.")); return; }
            ...
            SetStatus(UiText.F($"‘{target.Label}’ 적용 완료. 열린 Codex에서 계정을 확인하세요."), success: true);
            try { CodexAccountRuntime.LaunchDesktop(_desktopPath); }
            catch { SetStatus(UiText.F($"‘{target.Label}’은 적용되었습니다. 시작 메뉴에서 Codex를 열어주세요.")); }
    ...
    private void DeleteSelected()
    {
        ...
        if (MessageBox.Show(this, UiText.F($"‘{account.Label}’의 저장된 로그인을 삭제할까요?\n다시 사용하려면 해당 계정으로 로그인해야 합니다."), UiText.T("저장된 로그인 삭제"),
            MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) != DialogResult.OK) { SetStatus(UiText.T("삭제를 취소했습니다.")); return; }
        try { _store.Remove(account.Id); Reload(); _accountsChanged?.Invoke(); SetStatus(UiText.T("저장된 로그인을 삭제했습니다."), success: true); }
        catch (Exception ex) { SetStatus(ex.Message, error: true); }
    }
    ...
    private async Task RecoverAsync()
    {
        ...
        {
            await RunAsync(false, UiText.T("임시 파일을 정리하고 있습니다…"), async () =>
            {
                await Task.Run(_store.RetryUsageCleanup);
                Reload();
                SetStatus(_usageQueryStatus.State == UsageQueryState.None ? UiText.T("임시 파일 정리를 마쳤습니다.")
                    : UiText.T("임시 파일을 아직 정리하지 못했습니다. 잠시 후 다시 시도하세요."),
                    success: _usageQueryStatus.State == UsageQueryState.None);
            }, acquireGate: false);
            return;
        }
        await RunAsync(true, UiText.T("복구 조건을 확인하고 있습니다…"), async () =>
        {
            using var dialog = new AccountSwitchDialog("", "", recovery: true);
            if (dialog.ShowDialog(this) != DialogResult.OK) { SetStatus(UiText.T("복구를 취소했습니다. 미완료 기록은 보존됩니다.")); return; }
            ...
            Reload(); _accountsChanged?.Invoke(); SetStatus(UiText.T("중단된 계정 작업을 복구했습니다."), success: true);
            await Task.CompletedTask;
        });
    }
    ...
    {
        if (_busy) return;
        Present(() => { _busy = true; _progress.Visible = true; SetStatus(message); UpdateActions(); });
        ...
        finally
        {
            ...
            {
                try { _resume(); }
                catch { SetStatus(UiText.T("계정 작업은 끝났지만 사용량 조회를 재개하지 못했습니다. 위젯을 다시 열어주세요."), error: true); }
            }
    ...
    internal bool Reload(string? selectId = null, bool quiet = false)
    {
        try
        {
            ...
            _count.Text = UiText.F($"내 계정  {_items.Count}");
    ...
    {
        _operationMessage = UiText.Message(message); _operationError = error; _operationSuccess = success;
        RenderStatus();
    }

    private void RenderStatus() => Present(RenderStatusState);
    private void RenderStatusState()
    {
        var recovery = _store.HasPendingRecovery ? UiText.T("미완료 전환이 있습니다. 복구를 완료해 주세요.")
            : _usageQueryStatus.State == UsageQueryState.RecoveryRequired ? UiText.T("중단된 조회의 로그인 정보 복구가 필요합니다.") : null;
        var cleanup = _usageQueryStatus.State == UsageQueryState.CleanupPending
            ? UiText.T("로그인 정보 저장 완료 · 임시 파일 정리 대기") + (_usageQueryStatus.Failure is { } failure ? $" ({UiText.Message(failure.Summary)})" : "") : null;
        var fallback = _items.Count == 0 ? UiText.T("현재 계정을 먼저 등록하세요. 이름은 자동으로 지정됩니다.")
            : !_items.Any(a => a.IsActive) ? UiText.T("현재 로그인은 아직 등록되지 않았습니다. 전환하려면 현재 계정을 먼저 등록하세요.")
            : "";
        _status.Text = string.Join("\n", new[] { _operationMessage, recovery, cleanup, _items.Count > 0 && !_items.Any(a => a.IsActive) ? UiText.T("현재 로그인은 미등록 상태입니다. 먼저 등록해 주세요.") : null }.Where(text => text is not null));
    ...
    protected override CreateParams CreateParams
    {
        get
        {
            ...
            // Present the paper surfaces and their transparent native children as
            // one frame. Form.DoubleBuffered alone does not buffer child HWNDs.
            // Keep this local to the manager; the nonactivating widget/tooltip differ.
