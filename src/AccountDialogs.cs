...
internal sealed class AccountNameDialog : Form
{
    ...
    internal AccountNameDialog(string initialName, bool adding = false)
    {
        ...
        Text = adding ? UiText.T("다른 계정 추가") : UiText.T("계정 이름 변경");
        ...
        var description = AccountUiTheme.Label(adding
            ? UiText.T("공식 로그인 화면이 브라우저에서 열립니다.\n추가할 계정으로 로그인하세요. 현재 계정은 유지됩니다.")
            : UiText.T("알아보기 쉬운 이름을 붙여주세요.\n로그인 정보와 사용량에는 영향을 주지 않습니다."), color: AccountUiTheme.Muted);
        description.Dock = DockStyle.Fill;
        description.AutoSize = false;
        stack.Controls.Add(description, 0, 1);
        stack.Controls.Add(AccountUiTheme.Label(adding ? UiText.T("계정 이름 · 선택") : UiText.T("계정 이름")), 0, 2);
        _input.Text = initialName;
        _input.BackColor = AccountUiTheme.Raised; _input.ForeColor = AccountUiTheme.Text;
        _input.PlaceholderText = adding ? UiText.T("비우면 계정 번호로 지정됩니다") : UiText.T("1~40자");
        _input.TextChanged += (_, _) => _error.Text = "";
        stack.Controls.Add(new AccountInputFrame(_input), 0, 3);
        ...
        var save = AccountUiTheme.Button("SaveNameButton", adding ? UiText.T("브라우저에서 로그인") : UiText.T("저장"), true);
        var cancel = AccountUiTheme.Button("CancelNameButton", UiText.T("취소"));
        ...
        save.Click += (_, _) =>
        {
            ...
            {
                _error.Text = UiText.T("이름을 1~40자로 입력해 주세요. 줄바꿈은 사용할 수 없습니다.");
                _input.Focus();
                return;
            }
...
internal sealed class AccountSwitchDialog : Form
{
    ...
    private readonly Label _readiness = AccountUiTheme.Label(UiText.T("실행 중인 앱을 확인하고 있습니다…"));
    ...
    internal AccountSwitchDialog(string sourceName, string targetName, Action? assertWritersStopped = null, bool recovery = false)
    {
        ...
        AccountUiTheme.SetForm(this);
        Text = recovery ? UiText.T("중단된 계정 작업 복구") : UiText.T("계정 전환 준비");
        StartPosition = FormStartPosition.CenterParent;
        FormBorderStyle = FormBorderStyle.FixedDialog;
        MinimizeBox = false; MaximizeBox = false; ShowInTaskbar = false;
        ...
        stack.RowStyles.Add(new RowStyle(SizeType.Absolute, 42));
        stack.Controls.Add(AccountUiTheme.Label(Text, 17, true), 0, 0);
        var route = AccountUiTheme.Label(recovery ? UiText.T("저장된 복구 정보를 확인합니다") : $"{sourceName}  →  {targetName}", 12, true, AccountUiTheme.Accent);
        route.Dock = DockStyle.Fill; route.AutoSize = false; route.AutoEllipsis = true;
        stack.Controls.Add(route, 0, 1);
        var instructions = AccountUiTheme.Label(UiText.T("작업을 저장한 뒤 Codex 앱과 Codex 터미널·IDE 작업을\n직접 종료해 주세요. 앱이 닫히면 아래 버튼이 활성화됩니다.\n실행 중인 프로그램을 자동으로 종료하지 않습니다."), color: AccountUiTheme.Muted);
        ...
        var actions = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.RightToLeft, WrapContents = false, Margin = new Padding(0) };
        _confirm = AccountUiTheme.Button("ConfirmSwitchButton", recovery ? UiText.T("복구 확인") : UiText.T("전환하고 Codex 열기"), true);
        _confirm.Enabled = false;
        _confirm.Click += (_, _) => { if (CheckReadiness()) { DialogResult = DialogResult.OK; Close(); } };
        var cancel = AccountUiTheme.Button("CancelSwitchButton", UiText.T("취소"));
    ...
    private bool CheckReadiness()
    {
        try
        {
            ...
            _readiness.Text = UiText.T("● 준비 완료 · 모든 Codex 작업이 종료되었습니다.");
        ...
        {
            _readiness.Text = UiText.T("● 종료 대기 · Codex 앱 또는 관련 작업이 실행 중입니다.\n앱을 닫으면 자동으로 다시 확인합니다.");
